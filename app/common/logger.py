...
# 业务日志记录
dct_llm_context: ContextVar[dict] = ContextVar('dct_llm_context', default={
    'trace_id': '',
    'request_uri': '',
    'request_params': '',
    'llm_type': '',
...
log_path_root = Path(settings.LOG_PATH)
...
# 标准输出
logger.add(sys.stdout, level='INFO', format= "<green>{time:YYYY-MM-DD HH:mm:ss.SSS}</green> | {process.name} | {thread.name} |{trace_msg}| {level: <8} | {name}:{function}:{line} - {message}", filter=_logger_filter)
